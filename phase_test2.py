...
def main():
    ...
    for beat in tqdm(beat_list):
        ...
        for interval in tqdm(interval_list, leave=False):
            ...
            for max_length in tqdm(max_length_list, leave=False):
                ...
                results['data'][beat][interval][max_length]['energy_consumption'] = info['energy_penalty'].sum()
                # df = df.append({
                #     'action_interval': interval,
    ...
    with open(f'sim/analysis/phase_diagram1/data/without_energy/{swimming_way}_v2.json', mode='wt', encoding='utf-8') as f:
