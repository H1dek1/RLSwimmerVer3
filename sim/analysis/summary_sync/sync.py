...
def main():
    ...
    gs_master = GridSpec(
            ...
            ncols=4,
            height_ratios=[1, 1, 1],
            width_ratios=[1, 0.7, 1.5, 0.7],
            )
    gs_master.update(left=0.01,right=0.97,top=0.97,bottom=0.02,wspace=0.0,hspace=0.2)
    
    gs0 = GridSpecFromSubplotSpec(nrows=3, ncols=1, subplot_spec=gs_master[:,0])
    ax0 = fig.add_subplot(gs0[:,:])
    
    gs12 = GridSpecFromSubplotSpec(nrows=2, ncols=1, subplot_spec=gs_master[:2,2], hspace=0.3)
    ax1 = fig.add_subplot(gs12[0])
    ax2 = fig.add_subplot(gs12[1], projection='3d')
    gs3 = GridSpecFromSubplotSpec(nrows=1, ncols=1, subplot_spec=gs_master[2,1:])
    ...
    """
    plot_time = {}
    plot_time['sync'] = [0.0, 0.5, 1.0, 1.5, 2.0]
    for i in range(len(plot_time['sync'])):
        plot_time['sync'][i] += i * 40
    drawSwimmer(ax0, df, plot_time['sync'], ymin=0, ymax=20)
