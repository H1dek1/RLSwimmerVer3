...
def main():
    ...
    gs_master = GridSpec(
            ...
            ncols=3,
            height_ratios=[1, 1],
            width_ratios=[1.0, 2.0, 2.0],
            )
    gs_master.update(left=0.01,right=0.97,top=0.99,bottom=0.01,wspace=0.3,hspace=0.2)
    
    gs0 = GridSpecFromSubplotSpec(nrows=2, ncols=1, subplot_spec=gs_master[:,0])
    ax0 = fig.add_subplot(gs0[:,:])
    
    gs12 = GridSpecFromSubplotSpec(nrows=1, ncols=2, subplot_spec=gs_master[0,1:], hspace=0.2)
    ...
    df = pd.read_csv('../data/strategy_record/b_triangle_interval0.5_maxlength1.5.csv')
    """
    ax0
    """
    plot_time = {}
    plot_time['sync'] = [0.0, 0.5, 1.0, 1.5]
